...
# Maximum response size: 50MB - prevents OOM from malicious/buggy daemon
MAX_RESPONSE_SIZE = 50 * 1024 * 1024

# Client socket timeout: 30 seconds - prevents indefinite blocking
CLIENT_SOCKET_TIMEOUT = 30.0

# Windows lock size: must be consistent across lock/unlock calls
_WINDOWS_LOCK_SIZE = 32


def _get_lock_path(project: Path) -> Path:
    """Get lock file path for daemon startup synchronization."""
    hash_val = hashlib.sha256(str(project).encode()).hexdigest()[:16]
...
def _try_acquire_pidfile_lock(pid_path: Path) -> Optional[IO]:
    ...
    try:
        ...
        if sys.platform == "win32":
            ...
            # Lock _WINDOWS_LOCK_SIZE bytes for robustness
            try:
                msvcrt.locking(pidfile.fileno(), msvcrt.LK_NBLCK, _WINDOWS_LOCK_SIZE)
                return pidfile
            except IOError:
                pidfile.close()
                return None
